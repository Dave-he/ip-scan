...
开放端口通过数据库作为耐久化边界。扫描器可以继续生产；Connect scanner 在端口分发阶段也施加有界 JoinSet 背压，即使扫描 1-65535 也不会瞬间创建数万任务。enrichment worker 每秒选取尚未补充的记录；Geo 与服务探测各自受信号量限制。写入使用幂等 UPSERT，进程中断后下一轮会继续补偿。
